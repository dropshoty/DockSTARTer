...
install_compose() {
    ...
    if [[ ${AVAILABLE_COMPOSE} != "${INSTALLED_COMPOSE}" ]] || [[ -n ${FORCE} ]]; then
        ...
        pip install -IU pip > /dev/null 2>&1 || fatal "Failed to install latest pip."
        pip uninstall docker-py > /dev/null 2>&1 || true
        pip install -IU "urllib3[secure]" > /dev/null 2>&1 || fatal "Failed to install latest dependencies from pip."
